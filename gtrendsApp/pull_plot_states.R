...
for(i in 1:length(gtrend_keywords)){
  ...
  for(x in 1:length(gtrend_geos)){
    last_90_c19<-gtrends(c(gtrend_keywords[i]), geo = c(gtrend_geos[x]), time = paste0("2019-11-01",Sys.Date()-1), gprop="web", hl="en-US",
...
data<- data %>%
  group_by(geo) %>% 
  mutate(dot_val = ifelse(date2==dot_date,mo_avg,NA))

### STATES AND US ----
p<-1
for(p in 1:length(gtrend_keywords)){
  tmp<-ggplot() + 
    geom_line(data = data %>% filter(keyword == gtrend_keywords[p] & !is.na(mo_avg) & geo!="US"), 
              mapping = aes(as.Date(date), mo_avg,color = geo))+
    geom_line(data = data %>% filter(keyword == gtrend_keywords[p] & !is.na(mo_avg) & geo=="US"), 
              mapping = aes(as.Date(date), mo_avg,color = geo),size =1.25)+
    geom_point(data = data %>% filter(keyword == gtrend_keywords[p] & !is.na(mo_avg) & geo!="US"),
